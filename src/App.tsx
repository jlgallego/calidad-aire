...
import { fetchAirStations } from './airQuality'
import './App.css'
...
const stations: Station[] = [
  { id: '04', name: 'Plaza Elíptica', area: 'Usera', x: 28, y: 70, index: 78, status: 'Desfavorable', color: '#d86c3d', values: [['NO₂', '78', 'µg/m³'], ['PM₂.₅', '19', 'µg/m³'], ['PM₁₀', '34', 'µg/m³'], ['O₃', '41', 'µg/m³']], series: [35,42,40,48,52,47,55,59,62,57,65,71,76,73,78,74,69,72,77,80,76,78,78,78] },
  { id: '08', name: 'Escuelas Aguirre', area: 'Retiro', x: 67, y: 41, index: 36, status: 'Moderada', color: '#e1a238', values: [['NO₂', '36', 'µg/m³'], ['PM₂.₅', '8', 'µg/m³'], ['PM₁₀', '16', 'µg/m³'], ['O₃', '73', 'µg/m³']], series: [25,29,27,31,30,33,37,34,38,36,33,35,39,42,40,37,35,32,34,36,38,36,35,36] },
  { id: '18', name: 'Farolillo', area: 'Carabanchel', x: 20, y: 45, index: 22, status: 'Buena', color: '#3b9c78', values: [['NO₂', '22', 'µg/m³'], ['PM₂.₅', '6', 'µg/m³'], ['PM₁₀', '12', 'µg/m³'], ['O₃', '61', 'µg/m³']], series: [18,20,17,21,19,24,22,20,23,21,19,20,22,24,23,22,21,19,22,23,21,20,22,22] },
  { id: '24', name: 'Casa de Campo', area: 'Moncloa', x: 38, y: 27, index: 18, status: 'Buena', color: '#3b9c78', values: [['NO₂', '18', 'µg/m³'], ['PM₂.₅', '5', 'µg/m³'], ['PM₁₀', '11', 'µg/m³'], ['O₃', '68', 'µg/m³']], series: [15,17,14,16,18,19,17,20,18,16,15,17,19,18,20,18,17,19,18,17,16,18,18,18] },
  { id: '35', name: 'Plaza Castilla', area: 'Chamartín', x: 74, y: 18, index: 28, status: 'Buena', color: '#3b9c78', values: [['NO₂', '28', 'µg/m³'], ['PM₂.₅', '7', 'µg/m³'], ['PM₁₀', '14', 'µg/m³'], ['O₃', '58', 'µg/m³']], series: [22,24,23,25,26,28,27,25,27,29,28,26,27,30,29,28,27,29,28,27,28,28,28,28] },
]
...
function App() {
  ...
  return <main>
    ...
    <section className="dashboard"><div className="map-panel"><div className="panel-heading"><div><p className="eyebrow">SITUACIÓN ACTUAL</p><h2>Mapa de estaciones</h2></div><span className="date-chip">DATOS CADA 20 MIN</span></div><div className="map"><div className="map-lines" />{stationData.map((station) => <button className={`station ${selectedId === station.id ? 'selected' : ''}`} style={{ left: `${station.x}%`, top: `${station.y}%`, '--station-color': station.color } as React.CSSProperties} onClick={() => setSelectedId(station.id)} key={station.id} aria-label={`Abrir estación ${station.name}`}><span>{station.index}</span><small>{station.name}</small></button>)}</div><div className="legend"><span><i className="good" />Buena</span><span><i className="medium" />Moderada</span><span><i className="bad" />Desfavorable</span><span className="source">● {stationData.length} estaciones con datos</span></div></div>
      <aside className="detail-panel"><div className="detail-head"><div><p className="eyebrow">ESTACIÓN {selected.id}</p><h2>{selected.name}</h2><p className="muted">{selected.area} · Madrid</p></div><span className="status" style={{ '--station-color': selected.color } as React.CSSProperties}><i />{selected.status}</span></div><div className="score"><strong>{selected.index}</strong><span>Índice de calidad<br />del aire</span><div className="score-ring" style={{ '--station-color': selected.color } as React.CSSProperties} /></div><div className="measurements">{selected.values.map(([label, value, unit]) => <div key={label}><b>{label}</b><strong>{value}</strong><span>{unit}</span></div>)}</div><div className="detail-chart"><div className="chart-title"><span>LECTURAS · ÚLTIMAS 24 HORAS</span><b>NO₂</b></div><TrendChart data={selected.series} /></div><p className="alert"><span>!</span>{selected.status === 'Desfavorable' ? 'Superación del umbral horario de NO₂' : 'Sin superaciones registradas hoy'}</p></aside>
    </section>
    <section className="indicators"><div className="section-heading"><div><p className="eyebrow">RESUMEN DE LA RED</p><h2>Indicadores principales</h2></div><button className="outline-button">Ver histórico <span>↗</span></button></div><div className="indicator-grid"><article><span className="indicator-icon teal">◌</span><p>ESTACIONES ACTIVAS</p><strong>24 <small>/ 24</small></strong><span className="trend up">↑ 100% cobertura</span></article><article><span className="indicator-icon orange">≈</span><p>ÍNDICE MEDIO</p><strong>34 <small>ICA</small></strong><span className="trend">Dentro de lo esperado</span></article><article><span className="indicator-icon red">!</span><p>SUPERACIONES HOY</p><strong>3 <small>episodios</small></strong><span className="trend warning">2 estaciones afectadas</span></article><article><span className="indicator-icon blue">◷</span><p>ÚLTIMA ACTUALIZACIÓN</p><strong>12:40 <small>h</small></strong><span className="trend up">● Datos en tiempo real</span></article></div></section>
